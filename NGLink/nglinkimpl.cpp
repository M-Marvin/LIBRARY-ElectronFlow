...
void nglink_impl::logPrinter(string log)
{
...
bool nglink_impl::initNGLink(log_func printFunc, detach_func detacheFunc)
{
	return initNGLinkFull(printFunc, detacheFunc, NULL, NULL);
}
bool nglink_impl::initNGLinkFull(log_func printFunc, detach_func detacheFunc, receive_vec_data_func reciveVecFunc, receive_init_data_func reciveInitFunc)
{
...
bool nglink_impl::isInitialisated() {
	return initialised;
}

/* Try to load and attach the ngspice lib */
bool nglink_impl::initNGSpice(string libName)
{
	if (!initialised)
	{
		...
		else
		{
			...
			int status = ngSpiceInit(
				[](char* print, int id, void* caller) -> int {
					nglink_impl* nglinki = (nglink_impl*) caller;
				...
				[](pvecvaluesall data, int count, int id, void* caller) -> int {
					nglink_impl* nglinki = (nglink_impl*) caller;
					if (nglinki->reciveVecData != NULL) nglinki->reciveVecData((spice_pvecvaluesall) data);
					return 1;
				},
				[](pvecinfoall data, int id, void* caller) -> int {
					nglink_impl* nglinki = (nglink_impl*) caller;
					if (nglinki->reciveInitData != NULL) nglinki->reciveInitData((spice_pvecinfoall) data);
					return 1;
				},
				NULL, this
			);
...
bool nglink_impl::isNGSpiceAttached()
{
	return ngSpiceModule != 0;
}

/* Detaches the ngspice lib */
bool nglink_impl::detachNGSpice()
{
	if (!initialised)
	{
		logPrinter("nglinker is not initialized, use initNGLink first!");
...
bool nglink_impl::execCommand(string command) {
...
bool nglink_impl::loadCircuit(string circListString) {
	int status = checkNGState();
...
vector<string> nglink_impl::listPlots() {
	int status = checkNGState();
...
string nglink_impl::getCurrentPlot() {
	int status = checkNGState();
	if (status != 1) return "";
...
vector<string> nglink_impl::listVecs(string plotName) {
	int status = checkNGState();
	if (status != 1) return vector<string>();
