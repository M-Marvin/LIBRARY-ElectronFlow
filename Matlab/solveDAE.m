...
function [T,X] = solveDAE(E,A,zfun,t0,t1,h,x0)
    ...
    if isempty(x0)
        z0 = zfun(t0);
        % Solve A*x0 = z0 (A assumed invertible per top-level docs; if nearly singular use least squares)
        if rcond(A) < eps
            x0 = A \ z0; % will give least-squares if singular
        else
            x0 = A \ z0;
        end
    end
...
% Validate
if nargin<7, error('All 7 inputs required.'); end
n = size(E,1);
assert(all(size(E)==[n,n]) && all(size(A)==[n,n]) ,'E and A must be square same size');
assert(size(x0,1)==n && size(x0,2)==1,'x0 must be n-by-1');
...
Ad = AAp(1:nd,1:nd); Ed = EEp(1:nd,1:nd);
