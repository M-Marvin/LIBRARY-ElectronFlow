...
public class Voltage2Voltage extends FourPort {
	...
	public Voltage2Voltage(String name, int nodeA, int nodeB, int nodeC, int nodeD, double factor) {
	...
	public static Optional<Component> tryParse(String[] args, Function<String, Integer> nodeIdProvider) {
		if (args[0].startsWith("VUU") && args.length == 6) {
			...
			return Optional.of(new Voltage2Voltage(args[0], nodeA, nodeB, nodeC, nodeD, value));
		}
	...
	public String configInfo() {
		return String.format("%.03f V/V", this.factor);
	}
	...
	public void stampMatricies(Context ctx, MatrixNd A, MatrixNd E, MatrixNd z) {
		...
		if (this.nodeA != 0)
			A.addM(this.nodeA - 1, mid, -this.factor);
		if (this.nodeB != 0)
			A.addM(this.nodeB - 1, mid, +this.factor);
		if (this.nodeC != 0) {
			A.addM(mid, this.nodeC - 1, +1.0);
			A.addM(this.nodeC - 1, mid, +1.0);
		}
		if (this.nodeD != 0) {
			A.addM(mid, this.nodeD - 1, -1.0);
			A.addM(this.nodeD - 1, mid, -1.0);
		}
